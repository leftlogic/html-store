...
htmldb.parse = function (raw) {
  var $        = cheerio.load(raw),
      $meta    = $('meta'),
      $script  = $('script[data-name][type=text]'),
      file     = Object.create(htmldb.file),
      metadata = {},
      data     = {};
  ...
  $meta.each(function (i, elem) {
    var $elem = $(elem);
    metadata[$elem.attr('name')] = $elem.attr('content');
    $elem.remove();
  });
  file.meta(metadata);

  // Pull data from script tags
